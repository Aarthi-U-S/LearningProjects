...
public class RateLimitService : IRateLimitService
{
    ...
    public async Task ResetRateLimitAsync(string key)
    {
        ...
        // Reset rate limit data in the store
        // The store should handle clearing all algorithm-specific entries
    ...
    private static string GenerateKey(string identifier, string endpoint, RateLimitStrategy strategy)
    {
        ...
        {
            RateLimitStrategy.PerIp => identifier,
            RateLimitStrategy.PerUser => identifier,
            RateLimitStrategy.PerEndpoint => endpoint,
            RateLimitStrategy.PerIpAndEndpoint => $"{identifier}:{endpoint}",
            RateLimitStrategy.PerUserAndEndpoint => $"{identifier}:{endpoint}",
            _ => identifier
        };
    }
}
